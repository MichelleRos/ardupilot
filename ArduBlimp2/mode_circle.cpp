...
void ModeCircle::run()
{
    ...
    if (!blimp.failsafe.radio && blimp.circle_nav->pilot_control_enabled()) {
        // update the circle controller's radius target based on pilot pitch stick inputs
        const float radius_current = blimp.circle_nav->get_radius();           // circle controller's radius target, which begins as the circle_radius parameter
        const float pitch_stick = channel_pitch->norm_input_dz();               // pitch stick normalized -1 to 1
        const float nav_speed = blimp.wp_nav->get_default_speed_xy();          // blimp WP_NAV parameter speed
        ...
        if (!is_equal(radius_current, radius_new)) {
            blimp.circle_nav->set_radius(radius_new);
        }
        ...
        if (g.radio_tuning != TUNING_CIRCLE_RATE) {
            ...
            if (is_zero(roll_stick)) {
                ...
                const float rate = blimp.circle_nav->get_rate();           // circle controller's rate target, which begins as the circle_rate parameter
                const float rate_current = blimp.circle_nav->get_rate_current(); // current adjusted rate target, which is probably different from _rate
                ...
                speed_changing = true;
                blimp.circle_nav->set_rate(rate_new);
            }
        }
    }
    ...
    if (pilot_yaw_override) {
        attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(blimp.circle_nav->get_roll(),
                                                                      blimp.circle_nav->get_pitch(),
                                                                      target_yaw_rate);
    } else {
        attitude_control->input_euler_angle_roll_pitch_yaw(blimp.circle_nav->get_roll(),
                                                           blimp.circle_nav->get_pitch(),
                                                           blimp.circle_nav->get_yaw(), true);
    }

...
void Copter::update_ground_effect_detector(void)
{
    ...
    if (!throttle_up && ap.land_complete) {
        ...
        gndeffect_state.takeoff_alt_cm = pos_neu.z;
    }

    // if we are in takeoff_expected and we meet the conditions for having taken off
    // end the takeoff_expected state
    if (gndeffect_state.takeoff_expected && (tnow_ms-gndeffect_state.takeoff_time_ms > 5000 || pos_neu.z-gndeffect_state.takeoff_alt_cm > 50.0f)) {
        gndeffect_state.takeoff_expected = false;
    }
    ...
    bool z_speed_low = fabsf(vel_neu.z) <= 60.0f;

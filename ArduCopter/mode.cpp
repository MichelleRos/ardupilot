    ...
    if (!pause_descent) {
        ...
        if (doing_precision_landing) {
            ...
            if (copter.precland.get_target_position_cm(target_pos)) {
                const Vector2f current_pos = copter.pos_neu.xy();
...
void Mode::land_run_horizontal_control()
{
    ...
    if (doing_precision_landing) {
        ...
        if (!copter.precland.get_target_position_cm(target_pos)) {
            target_pos.x = copter.pos_neu.x;
            target_pos.y = copter.pos_neu.y;
        }
        if (!copter.precland.get_target_velocity_relative_cms(target_vel_rel)) {
            target_vel_rel.x = -copter.vel_neu.x;
            target_vel_rel.y = -copter.vel_neu.y;
        }

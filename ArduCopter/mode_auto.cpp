...
void ModeAuto::circle_movetoedge_start(const Location &circle_center, float radius_m)
{
    ...
    float dist_to_edge = (copter.pos_neu - circle_edge_neu).length();
    ...
    if (dist_to_edge > 300.0f) {
        ...
        const Vector3f &curr_pos = copter.pos_neu;
...
bool ModeAuto::verify_payload_place()
{
    ...
    case PayloadPlaceStateType_Descending_Start:
        ...
        nav_payload_place.descend_start_altitude = copter.pos_neu.z;
    ...
    case PayloadPlaceStateType_Descending:
        ...
        debug("descended: %f cm (%f cm max)", (nav_payload_place.descend_start_altitude - copter.pos_neu.z), nav_payload_place.descend_max);
        if (!is_zero(nav_payload_place.descend_max) &&
            nav_payload_place.descend_start_altitude - copter.pos_neu.z  > nav_payload_place.descend_max) {
    ...
    case PayloadPlaceStateType_Released: {
        ...
        FALLTHROUGH;
    case PayloadPlaceStateType_Ascending_Start: {
        Location target_loc(copter.pos_neu, Location::AltFrame::ABOVE_ORIGIN);

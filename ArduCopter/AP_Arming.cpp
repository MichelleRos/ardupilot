...
bool AP_Arming_Copter::barometer_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_BARO)) {
        ...
        copter.ahrs.get_filter_status(filt_status);
        bool using_baro_ref = (!filt_status.flags.pred_horiz_pos_rel && filt_status.flags.pred_horiz_pos_abs);
        if (using_baro_ref) {
            if (fabsf(copter.pos_neu.z - copter.baro_alt) > PREARM_MAX_ALT_DISPARITY_CM) {
...
bool AP_Arming_Copter::arm(const AP_Arming::Method method, const bool do_arming_checks)
{
    ...
    if (!ahrs.home_is_set()) {
        ...
        copter.arming_altitude_m = copter.pos_neu.z * 0.01;
    }

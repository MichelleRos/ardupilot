...
void ModeQStabilize::update()
{
    ...
    if (!plane.quadplane.option_is_set(QuadPlane::OPTION::INGORE_FW_ANGLE_LIMITS_IN_Q_MODES)) {
        ...
        plane.nav_roll_cd = roll_input * plane.quadplane.aparm.angle_max_deg*100.0;
        plane.nav_pitch_cd = pitch_input * plane.quadplane.aparm.angle_max_deg*100.0;
    }
}
...
{
    plane.nav_roll_cd = roll_input * MIN(plane.roll_limit_cd, plane.quadplane.aparm.angle_max_deg*100.0);
    // pitch is further constrained by PTCH_LIM_MIN/MAX which may impose
    // tighter (possibly asymmetrical) limits than Q_ANGLE_MAX
    if (pitch_input > 0) {
        plane.nav_pitch_cd = pitch_input * MIN(plane.aparm.pitch_limit_max*100, plane.quadplane.aparm.angle_max_deg*100.0);
    } else {
        plane.nav_pitch_cd = pitch_input * MIN(-plane.pitch_limit_min*100, plane.quadplane.aparm.angle_max_deg*100.0);
    }
}

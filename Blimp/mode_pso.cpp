...
void ModePSO::run()
{
    ...
    for (int i=0; i<PAR_MAX; i++){
        V[i].x = w*V[i].x + cc1*rr1*(pbest[i].x - X[i].x) + cc2*rr2*(pbest[gbest].x - X[i].x);// + d*A[i].x;
        V[i].y = w*V[i].y + cc1*rr1*(pbest[i].y - X[i].y) + cc2*rr2*(pbest[gbest].y - X[i].y);// + d*A[i].y;
    ...
    AP::logger().WriteStreaming("PSOT", "TimeUS,tpx,tpy", "Qff",
                                    AP_HAL::micros64(),

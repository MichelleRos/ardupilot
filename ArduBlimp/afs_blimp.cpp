/*
  blimp specific AP_AdvancedFailsafe class
 */
...
// Constructor
AP_AdvancedFailsafe_Blimp::AP_AdvancedFailsafe_Blimp(AP_Mission &_mission) :
 ...
 */
void AP_AdvancedFailsafe_Blimp::terminate_vehicle(void)
{
    if (_terminate_action == TERMINATE_ACTION_LAND) {
        blimp.set_mode(Mode::Number::LAND, ModeReason::TERMINATE);
    } else {
        // stop motors
        ...
        // disarm as well
        blimp.arming.disarm(AP_Arming::Method::AFS);
...
void AP_AdvancedFailsafe_Blimp::setup_IO_failsafe(void)
{
    ...
    uint16_t mask = blimp.motors->get_motor_mask();
    hal.rcout->set_failsafe_pwm(mask, blimp.motors->get_pwm_output_min());
#endif
}
 ...
 */
AP_AdvancedFailsafe::control_mode AP_AdvancedFailsafe_Blimp::afs_mode(void)
{
